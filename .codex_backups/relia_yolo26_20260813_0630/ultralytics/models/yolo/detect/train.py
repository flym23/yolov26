...
from ultralytics.nn.tasks import DetectionModel, ReLiADetectionModel
...
class ReLiADetectionTrainer(DetectionTrainer):
    """Detection trainer that binds a persisted URPC training-statistics snapshot to PWD."""

    def get_model(self, cfg: str | None = None, weights: str | None = None, verbose: bool = True):
        """Build RELiA, validate its immutable PWD statistics input, and load standard YOLO26 weights."""
        if not self.args.relia_stats:
            raise ValueError("RELiA training requires the persisted relia_stats JSON generated from the train split.")
        model = self.set_model_names_for_load(
            ReLiADetectionModel(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        )
        model.configure_paired_degradation(self.args.relia_stats)
        if weights:
            model.load(weights)
        return model
