...
class RelativeDegradationPromptAdapter(nn.Module):
    ...
    def forward(self, p2: torch.Tensor, p3: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, dict[str, torch.Tensor]]:
        ...
        code, logits_p3, logits_p4 = self.encoder(p2, p3.shape[-2:])
        severity_p3, severity_p4 = logits_p3.sigmoid(), logits_p4.sigmoid()
        return self.p2_adapter(p2, severity_p3, code), self.p3_adapter(p3, severity_p3, code), {"code": code, "severity_p3": logits_p3, "severity_p4": logits_p4, "map_p3": severity_p3, "map_p4": severity_p4}
...
class CDRStem(Conv):
    """Chromatic degradation and detail-reliability stem with pretrained Conv/BN compatibility."""
    ...
    def __init__(
        ...
        use_dual_highpass: bool = True,
        use_reliability: bool = True,
        use_router: bool = True,
        zero_init: bool = True,
    ) -> None:
        """Initialize the baseline-compatible stem and its zero-scaled auxiliary experts."""
        if c1 != 3:
            raise ValueError(f"CDRStem requires a 3-channel RGB input, got c1={c1}.")
        if s != 2:
            raise ValueError(f"CDRStem is designed for stride=2, got s={s}.")
        super().__init__(c1, c2, k, s, p, g, d, act)
        hidden = make_divisible(max(int(hidden), 8), 8)
        self.use_opponent = bool(use_opponent)
        self.use_dual_highpass = bool(use_dual_highpass)
        self.use_reliability = bool(use_reliability)
        self.use_router = bool(use_router)
        self.reliability_head = (
            nn.Sequential(Conv(9, hidden, 3, 1), nn.Conv2d(hidden, 1, 1, bias=True)) if self.use_reliability else None
        )
        self.context_branch = nn.Sequential(Conv(3, c2, 3, 2), DWConv(c2, c2, 3, 1, d=2))
        self.detail_branch = Conv(3, c2, 3, 2)
        self.router = (
            nn.Sequential(nn.Conv2d(6, hidden, 1, bias=True), nn.SiLU(), nn.Conv2d(hidden, 2, 1, bias=True))
            if self.use_router
            else None
        )
        gamma_init = 0.0 if zero_init else 1.0
        self.gamma_context = nn.Parameter(torch.full((1,), gamma_init))
        self.gamma_detail = nn.Parameter(torch.full((1,), gamma_init))
    ...
    @staticmethod
    def _opponent_color(x: torch.Tensor) -> torch.Tensor:
        """Return luminance and two chromatic-opponent channels."""
        r, g, b = x.chunk(3, dim=1)
        return torch.cat(((r + g + b) / 3.0, r - g, b - g), dim=1)

    def _base_forward(self, x: torch.Tensor, fused: bool) -> torch.Tensor:
        """Run the inherited convolution before or after BN fusion."""
        return self.act(self.conv(x)) if fused else self.act(self.bn(self.conv(x)))

    def _forward_impl(self, x: torch.Tensor, fused: bool) -> torch.Tensor:
        """Apply CDR logic for either the unfused or fused Conv path."""
        if x.ndim != 4 or x.shape[1] != 3:
            raise ValueError(f"CDRStem expects BCHW RGB input, got shape={tuple(x.shape)}.")
        base = self._base_forward(x, fused=fused)
        opponent = self._opponent_color(x) if self.use_opponent else x
        low3 = F.avg_pool2d(opponent, kernel_size=3, stride=1, padding=1)
        low5 = F.avg_pool2d(opponent, kernel_size=5, stride=1, padding=2)
        high3 = opponent - low3 if self.use_dual_highpass else opponent
        high5 = opponent - low5 if self.use_dual_highpass else high3
        ...
        if context.shape != base.shape or detail.shape != base.shape:
            raise RuntimeError(
                "CDRStem branch shape mismatch: "
                f"base={tuple(base.shape)}, context={tuple(context.shape)}, detail={tuple(detail.shape)}."
            )
        if self.router is None:
            router_weight = base.new_full((base.shape[0], 2, 1, 1), 0.5)
        else:
            router_input = torch.cat((low5, high3.abs()), dim=1)
            router_weight = self.router(F.adaptive_avg_pool2d(router_input, 1)).softmax(dim=1)
        gamma_context = torch.tanh(self.gamma_context).to(dtype=base.dtype)
        gamma_detail = torch.tanh(self.gamma_detail).to(dtype=base.dtype)
        return (
            base
            + gamma_context * router_weight[:, 0:1] * (context - base)
            + gamma_detail * router_weight[:, 1:2] * reliability * detail
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Run CDR-Stem before Conv-BN fusion."""
        return self._forward_impl(x, fused=False)

    def forward_fuse(self, x: torch.Tensor) -> torch.Tensor:
        """Run CDR-Stem after the inherited Conv-BN pair has been fused."""
        return self._forward_impl(x, fused=True)


class SDRFusion(nn.Module):
    """Subpixel detail-routing fusion for the P2-to-P3 path."""
    ...
    def __init__(
        ...
        """Initialize P2 compression, four-way routing, and zero-scaled detail injection."""
        super().__init__()
        if len(ch) != 3:
            raise ValueError(f"SDRFusion expects exactly three input channel values, got ch={ch}.")
        if not 0.0 < float(reduce_ratio) <= 1.0:
            raise ValueError(f"reduce_ratio must be in (0, 1], got {reduce_ratio}.")
        if mode not in {"full", "route", "stride_concat", "pixel_concat"}:
        ...
        self.out_channels = {
            "full": self.c_td + self.c_p3,
            "route": self.c_td + self.c_p3,
            "stride_concat": self.c_td + 2 * self.c_p3,
            "pixel_concat": self.c_td + self.c_p3 + 4 * self.c_red,
        }[mode]
        route_hidden = make_divisible(max(16, (self.c_td + self.c_p3) // 8), 8)
        gate_hidden = make_divisible(max(16, self.c_p3 // 4), 8)
        self.p2_stride = Conv(self.c_p2, self.c_p3, 3, 2) if mode == "stride_concat" else None
        self.p2_reduce = Conv(self.c_p2, self.c_red, 1, 1) if mode != "stride_concat" else None
        self.group_project = (
            nn.Sequential(Conv(self.c_red, self.c_p3, 1, 1), DWConv(self.c_p3, self.c_p3, 3, 1))
            if mode in {"full", "route"}
            else None
        )
        self.route_context = Conv(self.c_td + self.c_p3, route_hidden, 1, 1) if mode in {"full", "route"} else None
        self.route_head = (
            nn.Sequential(Conv(route_hidden + 4, route_hidden, 3, 1), nn.Conv2d(route_hidden, 4, 1, bias=True))
            if mode in {"full", "route"}
            else None
        )
        self.gate_head = (
            nn.Sequential(Conv(self.c_p3 * 2 + 1, gate_hidden, 1, 1), nn.Conv2d(gate_hidden, 1, 1, bias=True))
            if mode in {"full", "route"} and self.use_reliability_gate
            else None
        )
    ...
    def pixel_unshuffle_groups(x: torch.Tensor, factor: int = 2) -> torch.Tensor:
        """Return pixel-unshuffled features as [B, factor², C, H/factor, W/factor]."""
        if x.ndim != 4:
            raise ValueError(f"pixel_unshuffle_groups expects BCHW input, got shape={tuple(x.shape)}.")
        if factor <= 0 or x.shape[-2] % factor or x.shape[-1] % factor:
            raise ValueError(f"Input spatial size {tuple(x.shape[-2:])} must be divisible by factor={factor}.")
        unshuffled = F.pixel_unshuffle(x, factor)
        batch, _, height, width = unshuffled.shape
        return unshuffled.reshape(batch, x.shape[1], factor * factor, height, width).permute(0, 2, 1, 3, 4).contiguous()

    def forward(self, x: list[torch.Tensor] | tuple[torch.Tensor, ...]) -> torch.Tensor:
        """Fuse [top-down P3, backbone P3, backbone P2] without adding a P2 detection head."""
        if not isinstance(x, (list, tuple)) or len(x) != 3:
            raise ValueError("SDRFusion expects [top_down_p3, backbone_p3, backbone_p2].")
        top_down, p3, p2 = x
        if top_down.ndim != 4 or p3.ndim != 4 or p2.ndim != 4:
            raise ValueError(
                "SDRFusion inputs must be BCHW tensors, got "
                f"top_down={tuple(top_down.shape)}, p3={tuple(p3.shape)}, p2={tuple(p2.shape)}."
            )
        if top_down.shape[1] != self.c_td or p3.shape[1] != self.c_p3 or p2.shape[1] != self.c_p2:
            raise ValueError(
                "SDRFusion channel mismatch: "
                f"expected=({self.c_td}, {self.c_p3}, {self.c_p2}), "
                f"got=({top_down.shape[1]}, {p3.shape[1]}, {p2.shape[1]})."
            )
        if top_down.shape[-2:] != p3.shape[-2:]:
            raise ValueError(
                f"SDRFusion requires equal P3 sizes, got top_down={tuple(top_down.shape[-2:])}, "
                f"backbone_p3={tuple(p3.shape[-2:])}."
            )
        expected_p2 = (p3.shape[-2] * 2, p3.shape[-1] * 2)
        if p2.shape[-2:] != expected_p2:
            raise ValueError(
                "SDRFusion expects P2 spatial size exactly 2x P3, "
        ...
        if self.mode == "pixel_concat":
            return torch.cat((top_down, p3, F.pixel_unshuffle(reduced_p2, 2)), dim=1)

        groups = self.pixel_unshuffle_groups(reduced_p2, factor=2)
        batch, group_count, _, height, width = groups.shape
        if group_count != 4:
            raise RuntimeError(f"SDRFusion expected four subpixel groups, got {group_count}.")
        projected = self.group_project(groups.reshape(batch * 4, self.c_red, height, width)).reshape(
            batch, 4, self.c_p3, height, width
        )
        group_energy = groups.abs().mean(dim=2)
        route_context = self.route_context(torch.cat((top_down, p3), dim=1))
        route_weight = self.route_head(torch.cat((route_context, group_energy), dim=1)).softmax(dim=1)
        detail = (projected * route_weight.unsqueeze(2)).sum(dim=1)
        group_std = group_energy.std(dim=1, keepdim=True, unbiased=False)
        gate = (
            torch.sigmoid(self.gate_head(torch.cat((p3, detail, group_std), dim=1)))
            if self.gate_head is not None
            else torch.ones_like(p3[:, :1])
        )
