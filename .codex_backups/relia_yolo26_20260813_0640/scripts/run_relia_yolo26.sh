...
if [[ ! -e $SMOKE_ROOT/state.json ]]; then
    ...
    "$PYTHON" "$SOURCE_ROOT/tools/urpc/run_relia_ablation.py" \
        --stage smoke --run-id "smoke_$RUN_ID" --project "$SMOKE_ROOT" --data "$DATA" --weights "$WEIGHTS" --relia-stats "$STATS" --ids A7
fi
...
[[ $SMOKE_STATUS == completed ]] || { echo "RELiA smoke check did not complete: $SMOKE_STATUS" >&2; exit 1; }
