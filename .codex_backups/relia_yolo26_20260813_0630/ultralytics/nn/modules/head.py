...
class DSQDetect(Detect):
    """Dual-branch shared-quality end-to-end detection head for RELiA-YOLO26."""

    def __init__(
        self, nc: int = 80, quality_alpha: float = 0.5, reg_max: int = 16, end2end: bool = False, ch: tuple = ()
    ) -> None:
        """Initialize quality logits for both native one-to-many and one-to-one branches."""
        if not 0.0 <= float(quality_alpha) <= 1.0:
            raise ValueError(f"DSQ quality_alpha must lie in [0, 1], got {quality_alpha}.")
        super().__init__(nc=nc, reg_max=reg_max, end2end=end2end, ch=ch)
        self.quality_alpha = float(quality_alpha)
        channels = max(16, ch[0] // 4)
        self.cv4 = nn.ModuleList(nn.Sequential(DWConv(c, c, 3), Conv(c, channels, 1), nn.Conv2d(channels, 1, 1)) for c in ch)
        self.one2one_cv4 = copy.deepcopy(self.cv4) if end2end else nn.ModuleList()

    def _quality(self, features: list[torch.Tensor], heads: nn.ModuleList) -> torch.Tensor:
        """Flatten quality logits in the exact native Detect anchor order."""
        batch = features[0].shape[0]
        return torch.cat([head(feature).view(batch, 1, -1) for head, feature in zip(heads, features)], 2)

    def forward(self, x: list[torch.Tensor]) -> dict[str, torch.Tensor] | torch.Tensor | tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Run twin native heads, retaining detached one-to-one features and both quality branches."""
        if not self.end2end:
            return super().forward(x)
        one2many = self.forward_head(x, **self.one2many)
        one2many["quality"] = self._quality(x, self.cv4)
        x_detach = [feature.detach() for feature in x]
        one2one = self.forward_head(x_detach, **self.one2one)
        one2one["quality"] = self._quality(x_detach, self.one2one_cv4)
    ...
    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        """Rescore one-to-one class probabilities before the inherited NMS-free top-k selection."""
        decoded = self._get_decode_boxes(x)
        scores = x["scores"].sigmoid()
        if self.quality_alpha == 0.0:
            return torch.cat((decoded, scores), 1)
        quality = x["quality"]
        if quality.shape != (scores.shape[0], 1, scores.shape[-1]):
            raise RuntimeError(f"DSQ quality-anchor mismatch: quality={tuple(quality.shape)}, scores={tuple(scores.shape)}.")
        return torch.cat((decoded, scores * quality.sigmoid().pow(self.quality_alpha)), 1)

    def bias_init(self) -> None:
        """Initialize native Detect biases and conservative quality priors for both branches."""
        super().bias_init()
        prior = math.log(0.1 / 0.9)
        for heads in (self.cv4, self.one2one_cv4):
            for head in heads:
                nn.init.normal_(head[-1].weight, mean=0.0, std=0.01)
                head[-1].bias.data.fill_(prior)

    def fuse(self) -> None:
        """Drop every one-to-many prediction module after end-to-end export fusion."""
        self.cv2 = self.cv3 = self.cv4 = None


class CCQDetect(Detect):
    """Cross-branch consensus quality head for end-to-end YOLO26 detection."""
    ...
    def __init__(
        ...
        """Initialize a Detect head with an additional quality branch for one-to-one predictions."""
        if float(quality_beta) <= 0:
            raise ValueError(f"quality_beta must be positive, got {quality_beta}.")
        super().__init__(nc=nc, reg_max=reg_max, end2end=end2end, ch=ch)
    ...
    def forward_quality(self, x: list[torch.Tensor]) -> torch.Tensor:
        ...
        if len(x) != self.nl:
            raise ValueError(f"CCQDetect expects {self.nl} feature levels, got {len(x)}.")
        batch_size = x[0].shape[0]
        return torch.cat([self.one2one_q[i](x[i]).view(batch_size, 1, -1) for i in range(self.nl)], dim=-1)

    def forward(
        self, x: list[torch.Tensor]
    ) -> dict[str, torch.Tensor] | torch.Tensor | tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Return YOLO26 one-to-many and one-to-one predictions with quality logits."""
        if not self.end2end:
            return super().forward(x)
        one2many = self.forward_head(x, **self.one2many)
        x_detach = [feature.detach() for feature in x]
    ...
    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        """Decode boxes and calibrate class scores without changing the output channel contract."""
        dbox = self._get_decode_boxes(x)
        scores = x["scores"].sigmoid()
    ...
    def bias_init(self):
        """Initialize Detect biases and a conservative prior for localization quality."""
        super().bias_init()
