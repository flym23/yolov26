...
def relia_image_statistics(images: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    laplace = -4 * luminance + F.pad(luminance[..., 1:, :], (0, 0, 0, 1)) + F.pad(luminance[..., :-1, :], (0, 0, 1, 0))
...
class PairedWeatherDegradation(nn.Module):
    ...
    def __init__(self, reference: ReLiAStatisticReference | None = None, max_retries: int = 3) -> None:
        ...
        self.register_buffer("ranges", torch.tensor((0.12, 0.12, 0.12, 0.06, 0.06, 0.06, 1.0, 0.035, 0.18)))
    ...
    def _apply(self, raw: torch.Tensor, delta: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, raw: torch.Tensor) -> PairedWeatherOutput:
        ...
        degraded, severity = self._apply(raw, delta)
        for _ in range(self.max_retries):
            ...
            delta[rejected] *= 0.5
            retries[rejected] += 1
            candidate, candidate_severity = self._apply(raw[rejected], delta[rejected])
...
class ReLiAE2ELoss(E2ELoss):
    ...
    def _native(self, prediction: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]):
        ...
        _, o2m_loss, _ = self.one2many.get_assigned_targets_and_loss(prediction["one2many"], batch)
        o2o_assignment, o2o_loss, detached = self.one2one.get_assigned_targets_and_loss(prediction["one2one"], batch)
        return (o2m_loss.sum() * self.o2m + o2o_loss.sum() * self.o2o) * prediction["one2one"]["boxes"].shape[0], detached, o2o_assignment
    ...
    def __call__(self, prediction: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], paired: dict[str, Any]):
        ...
        raw_native, raw_items, raw_assignment = self._native(raw_prediction, batch)
        degraded_native, degraded_items, _ = self._native(degraded_prediction, batch)
        ...
        if "quality" in raw_prediction["one2one"]:
            degraded_assignment = self.one2one.get_assigned_targets_and_loss(degraded_prediction["one2one"], batch)[0]
            value = 0.5 * (_relia_dsq_loss(raw_prediction["one2one"], raw_assignment, self.one2one, self.hard_negatives) + _relia_dsq_loss(degraded_prediction["one2one"], degraded_assignment, self.one2one, self.hard_negatives))
...
class CCQE2ELoss(E2ELoss):
    ...
    def __init__(self, model: torch.nn.Module):
        """Initialize the original progressive loss schedule and CCQ coefficients."""
        super().__init__(model)
        cfg = model.yaml.get("ccq", {}) or {}
        self.box_gain = float(cfg.get("box_gain", 0.5))
        self.quality_gain = float(cfg.get("quality_gain", 0.5))
        self.tau = float(cfg.get("tau", 0.2))
        self.delta = float(cfg.get("delta", 0.02))
        self.l1_gain = float(cfg.get("l1_gain", 0.25))
        self.hard_neg_topk = int(cfg.get("hard_neg_topk", 128))
        self.hard_neg_gain = float(cfg.get("hard_neg_gain", 0.25))
        self.use_consensus = bool(cfg.get("use_consensus", True))
        self.use_conditional_gate = bool(cfg.get("use_conditional_gate", True))
        self.use_reliability = bool(cfg.get("use_reliability", True))
        ...
        if head is not None and hasattr(head, "quality_beta") and not math.isclose(
            ...
            raise ValueError(
                ...
                f"head={head.quality_beta}, ccq={configured_beta}."
            )
        if head is not None and hasattr(head, "enable_quality") and bool(head.enable_quality) != self.use_quality:
            raise ValueError(
                "CCQ quality-head mismatch between YAML head args and ccq config: "
                f"head={head.enable_quality}, ccq={self.use_quality}."
            )
        if self.box_gain < 0 or self.quality_gain < 0:
            raise ValueError("CCQ loss gains must be non-negative.")
        if self.tau <= 0:
            raise ValueError(f"CCQ tau must be positive, got {self.tau}.")
        if self.delta < 0:
            raise ValueError(f"CCQ delta must be non-negative, got {self.delta}.")
        if self.hard_neg_topk < 0:
            raise ValueError(f"hard_neg_topk must be non-negative, got {self.hard_neg_topk}.")

    @staticmethod
    def _decode_branch(criterion, preds: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        """Decode a prediction branch into pixel-space boxes and anchor-major class logits."""
        pred_dist = preds["boxes"].permute(0, 2, 1).contiguous()
        anchor_points, stride_tensor = make_anchors(preds["feats"], criterion.stride, 0.5)
        boxes = criterion.bbox_decode(anchor_points, pred_dist) * stride_tensor
        return boxes, preds["scores"].permute(0, 2, 1).contiguous()

    def _consensus_teacher(
        self,
        pred_boxes: torch.Tensor,
        pred_scores: torch.Tensor,
        assignment: tuple[torch.Tensor, ...],
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor] | None:
        """Aggregate detached one-to-many positive boxes into one teacher per ground truth."""
        fg_mask, target_gt_idx, target_bboxes, *_ = assignment
        fg_mask = fg_mask.bool()
        if not bool(fg_mask.any()):
            return None
        batch_idx, anchor_idx = fg_mask.nonzero(as_tuple=True)
        n_max_boxes = self.one2many.assigner.n_max_boxes
        group_idx = batch_idx * n_max_boxes + target_gt_idx[batch_idx, anchor_idx].long()
        group_count = pred_boxes.shape[0] * n_max_boxes
        boxes = pred_boxes.detach()[batch_idx, anchor_idx]
        targets = target_bboxes[batch_idx, anchor_idx]
        confidence = pred_scores.detach()[batch_idx, anchor_idx].sigmoid().amax(dim=-1)
        iou = bbox_iou(boxes, targets, xywh=False).squeeze(-1).clamp_(0, 1)
        weight = confidence.clamp_min(self.eps).sqrt() * iou.pow(6) + self.eps
        weight_sum = boxes.new_zeros(group_count)
        box_sum = boxes.new_zeros(group_count, 4)
        target_sum = boxes.new_zeros(group_count, 4)
        count = boxes.new_zeros(group_count)
        weight_sum.index_add_(0, group_idx, weight)
        box_sum.index_add_(0, group_idx, boxes * weight.unsqueeze(-1))
        target_sum.index_add_(0, group_idx, targets * weight.unsqueeze(-1))
        count.index_add_(0, group_idx, torch.ones_like(weight))
        denominator = weight_sum.clamp_min(self.eps).unsqueeze(-1)
        teacher = box_sum / denominator
        target = target_sum / denominator
        deviation_sum = boxes.new_zeros(group_count, 4)
        scale = torch.cat(((target[:, 2:4] - target[:, 0:2]).clamp_min(1.0),) * 2, dim=-1)
        deviation_sum.index_add_(
            0, group_idx, ((boxes - teacher[group_idx]).abs() / scale[group_idx]) * weight.unsqueeze(-1)
        )
        deviation = deviation_sum / denominator
        reliability = (
            torch.exp(-deviation.mean(dim=-1) / self.tau).clamp_(0, 1)
            if self.use_reliability
            else torch.ones_like(count)
        )
        valid = (count >= 2) & torch.isfinite(teacher).all(dim=-1) & torch.isfinite(reliability)
        return teacher.detach(), reliability.detach(), valid.detach()

    def _ccq_box_loss(
        self,
        student_boxes: torch.Tensor,
        assignment: tuple[torch.Tensor, ...],
        teacher_data: tuple[torch.Tensor, torch.Tensor, torch.Tensor] | None,
    ) -> torch.Tensor:
        """Apply consensus distillation only when a reliable teacher outperforms the one-to-one student."""
        ...
        n_max_boxes = self.one2many.assigner.n_max_boxes
        group_idx = batch_idx * n_max_boxes + target_gt_idx[batch_idx, anchor_idx].long()
        teacher, reliability, teacher_valid = teacher_data
        student = student_boxes[batch_idx, anchor_idx]
        target = target_bboxes[batch_idx, anchor_idx]
        teacher_iou = bbox_iou(teacher[group_idx], target, xywh=False).squeeze(-1)
        student_iou = bbox_iou(student, target, xywh=False).squeeze(-1)
        valid = teacher_valid[group_idx]
        if self.use_conditional_gate:
            valid = valid & (teacher_iou > student_iou.detach() + self.delta)
        if not bool(valid.any()):
            return zero
        scale = torch.cat(((target[:, 2:4] - target[:, 0:2]).clamp_min(1.0),) * 2, dim=-1)
        iou_term = 1.0 - bbox_iou(student, teacher[group_idx], xywh=False).squeeze(-1)
        l1_term = ((student - teacher[group_idx]).abs() / scale).mean(dim=-1)
        weighted = reliability[group_idx] * valid.to(student.dtype) * (iou_term + self.l1_gain * l1_term)
    ...
    def _quality_loss(
        ...
        """Supervise positive localization quality and a bounded set of difficult negatives."""
        if not self.use_quality:
            return student_boxes.sum() * 0.0
        quality_logits = one2one["quality"].squeeze(1)
        fg_mask, target_gt_idx, target_bboxes, *_ = assignment
        fg_mask = fg_mask.bool()
        quality_loss = quality_logits.sum() * 0.0
        if bool(fg_mask.any()):
            batch_idx, anchor_idx = fg_mask.nonzero(as_tuple=True)
            target = target_bboxes[batch_idx, anchor_idx]
            quality_target = bbox_iou(student_boxes[batch_idx, anchor_idx].detach(), target, xywh=False).squeeze(-1)
            positive_bce = self.quality_bce(quality_logits[batch_idx, anchor_idx], quality_target.clamp_(0, 1))
            if teacher_data is None:
                quality_loss = positive_bce.mean()
            else:
                n_max_boxes = self.one2many.assigner.n_max_boxes
                group_idx = batch_idx * n_max_boxes + target_gt_idx[batch_idx, anchor_idx].long()
        ...
        if k == 0:
            return quality_loss
        negative_score = student_scores.sigmoid().amax(dim=-1).masked_fill(fg_mask, -1.0)
        negative_idx = negative_score.topk(k, dim=1).indices
        valid_negative = (~fg_mask).gather(1, negative_idx)
        negative_logits = quality_logits.gather(1, negative_idx)
        negative_loss = self.quality_bce(negative_logits, torch.zeros_like(negative_logits))
        negative_loss = (negative_loss * valid_negative).sum() / valid_negative.sum().clamp_min(1)
        return quality_loss + self.hard_neg_gain * negative_loss

    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Compute the original progressive losses and the two additional CCQ losses without repeating assignment."""
        preds = self.one2many.parse_output(preds)
        one2many, one2one = preds["one2many"], preds["one2one"]
        ...
        loss_vector = torch.cat(
            (
                base_loss * batch_size,
                (ccq_box_loss * self.box_gain * batch_size).reshape(1),
                (quality_loss * self.quality_gain * batch_size).reshape(1),
            )
        )
        return loss_vector, {
            **log_one,
            "ccq_box_loss": (ccq_box_loss * self.box_gain).detach(),
            "quality_loss": (quality_loss * self.quality_gain).detach(),
        }
