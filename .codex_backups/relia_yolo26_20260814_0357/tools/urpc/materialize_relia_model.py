...
def materialize(identifier: str, output: Path) -> Path:
    ...
    if identifier == "A0":
        return (ROOT / "ultralytics/cfg/models/26/yolo26.yaml").resolve()
    if identifier not in FLAGS:
        raise ValueError(f"RELiA experiment must be A0-A7, got {identifier}.")
    config = deepcopy(YAML.load(ROOT / "ultralytics/cfg/models/26/yolo26-relia.yaml"))
