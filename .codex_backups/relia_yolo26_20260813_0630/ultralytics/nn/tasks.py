...
class ReLiADetectionModel(DetectionModel):
    ...
    def __init__(self, cfg="yolo26-relia.yaml", ch=3, nc=None, verbose=True):
        ...
        self.enable_redpa = bool(self.relia.get("redpa", False))
        self.enable_psr = bool(self.relia.get("psr", False))
        self.enable_dsq = bool(self.relia.get("dsq", False))
    ...
    def _relia_forward(self, x: torch.Tensor, capture_aux: bool) -> tuple[Any, dict[str, dict[str, torch.Tensor]]]:
        ...
        for module in self.model:
            ...
            x = module(x)
            if module.i == self._p3_index:
                if self.enable_redpa:
                    saved[self._p2_index], x, prompt = self.redpa(saved[self._p2_index], x)
                    if capture_aux:
                    ...
                    prompt = {
                        ...
                        "map_p3": x.new_zeros(x.shape[0], 1, *x.shape[-2:]),
                        "map_p4": x.new_zeros(x.shape[0], 1, max(1, x.shape[-2] // 2), max(1, x.shape[-1] // 2)),
                    }
            saved.append(x)
        return x, context
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [c1, c2, *args[1:]]
            if m is CDRStem and args and isinstance(args[-1], dict):
                module_kwargs = args.pop()
            ...
            if len(in_ch) != 3:
                raise ValueError(f"SDRFusion expects exactly 3 inputs, got {len(in_ch)}")
            args = [in_ch, *args]
            if args and isinstance(args[-1], dict):
                module_kwargs = args.pop()
            reduce_ratio = float(args[1]) if len(args) > 1 else 0.25
            mode = str(module_kwargs.get("mode", "full"))
            c_red = make_divisible(max(16, int(in_ch[2] * reduce_ratio)), 8)
            c2 = {
                "full": in_ch[0] + in_ch[1],
                "route": in_ch[0] + in_ch[1],
                "stride_concat": in_ch[0] + 2 * in_ch[1],
                "pixel_concat": in_ch[0] + in_ch[1] + 4 * c_red,
            }.get(mode)
            if c2 is None:
            ...
            if m is CCQDetect:
                ...
                args.extend([enable_quality, reg_max, end2end, [ch[x] for x in f]])
            else:
        ...
        m_ = torch.nn.Sequential(*(m(*args, **module_kwargs) for _ in range(n))) if n > 1 else m(*args, **module_kwargs)
